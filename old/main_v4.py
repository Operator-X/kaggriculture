...
def agent(observation, configuration=None):
    ...
    for y in range(10):
        for x in range(10):
            ...
            if tile is None:
                ...
                # Limit planting hours to hour < 18 to ensure we water it on day 0
                # If we have seeds in stock, we schedule planting tasks
                if hour < 18:
                    remaining_days = 28 - day
                    eligible_crops = [
                        crop for crop, count in private["seeds"].items()
    ...
    if hour == 0 and day < 29:
        ...
        # Bounds: x_c >= 0, 0 <= s_c <= private["seeds"][c]
        for crop in crop_names:
            ...
            if remaining_days < crop_data["max_yield_day"]:
                ...
                bounds.append((0.0, None)) # x_c upper bound = infinity (diversity checked in A_ub)
        ...
        for crop in crop_names:
            ...
            # Penalize glut risk for high-volume crops
        ...
        for y in range(max_hands_limit + 1):
            ...
            for j, crop in enumerate(crop_names):
                max_allowed = max(3.0, total_unlocked_tiles * 0.40)
                b_ub.append(max(0.0, max_allowed - crop_counts[crop]))
            
            try:
                res = linprog(c_coef, A_ub=A_ub, b_ub=b_ub, bounds=bounds, method="highs")
                if res.success:
                    # Extract the continuous choices for seed buys x_opt
                    x_opt = res.x[:5]
                    # Numerical safety buffer (+1e-7) to avoid precision errors rounding down
                    round_seeds = [math.floor(val + 1e-7) for val in x_opt]
                    
                    # Round down s_opt as well for profit calculation
                    s_opt = res.x[5:]
                ...
                # Fail-safe: if solver encounters error, skip
                continue
